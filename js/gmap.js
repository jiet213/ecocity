...
function gMap(containerId, m) {
	...
	//控制缩放效果
	this.scaleParames={'zoomfactor':1,'zoomtime':100,'centerPt':{'x':0,'y':0}};
...
gMap.prototype.init = function(m) {
	...
	this.baseMapDiv.style.margin = "0px";
	this.baseMapDiv.style.padding = "0px";
	...
	this.mapView.style.margin = "0px";
	this.mapView.style.padding = "0px";
	...
	this.dynMap.style.left = '0px';
	this.dynMap.style.top ='0px';
...
if(EventUtil.getIEversion()!=0){
 ...
 if(EventUtil.getIEversion()!=0){
	...
	//this.webMapURL = "../";
	//this.webHost="http://58.198.182.28";
	//this.fileServer = "../FileServer?fn=";
	//this.fileServer = "http://58.198.182.28:8090/FileServer?fn=";
	...
	this.fileServer = "http://58.198.182.28:81/FileServer?fn=";
	this.fileServer1 = this.webHost + ":81/FileServer?fn=";
	...
	if (!isMobile) {
		...
		if (startName in doc) {
		    ...
		    EventUtil.addHander(this.container,'touchmove',mapMouseMoveEvt);
		    EventUtil.addHander(this.container,'touchend',mapMouseUpEvt);
		   // EventUtil.addHander(this.container,'touchend',mapMouseUpEvtEx);
...
gMap.prototype.requestTileMap = function(params) {
    ...
    var mapurl = 'http://58.198.182.28:81/TileMap';
    //console.log("切片参数："+JSON.stringify(params));
...
gMap.prototype.requestMap = function(params) {
   ...
   // var reqUrl = "http://localhost:8090/webmap?map=shxz2008&&w=" + mapParams.w + "&h=" + mapParams.h + "&cx=" + mapParams.cx + "&cy=" + mapParams.cy + "&zoom=" + mapParams.zoom + "&lyrs=" + mapParams.lyrs + "&mt=" + mapParams.mt + "&return=json";
    //console.log(reqUrl);
   // console.log("动态图参数："+JSON.stringify(params));
   /* var mcx= mapParams.cx.toFixed(5);
    var mcy= mapParams.cy.toFixed(5);
    var tcx= tileParams.cx.toFixed(5);
    var tcy= tileParams.cy.toFixed(5);
    if(mcx!=tcx||mcy!=tcy){
        alert("出现问题");
	}*/
    try {
        ajax("get", mapurl, params, false, function(data) {
            ...
            gSelf.mapImg.src = gSelf.fileServer1 + data.mapURL;
...
gMap.prototype.showMap = function(data) {
	...
	if (this.ifInit || this.ifZoomTo || (this.maptool == "zoomin") || (this.maptool == "zoomout")||(this.maptool == "zoomMap")) {
	...
	}else if(this.maptool == "zoomMap"){//主要是为了隐藏影像图层之后，再进行影像图层的显示
      this.resetTileInit();
	}
	var tmpIndexArr = [];
	...
	var j = 0;
	var i = 0;
    var responseCenterx=data.tileInfo.cx;
    var responseCentery=data.tileInfo.cy;
	function getImg() {
		...
		if (!(gSelf.hasImgURL[imgtmp.id]) && tmpzl == gSelf.zoomLevel&&responseCenterx==tileParams.cx&&tileParams.cy==responseCentery) {
			...
			imgtmp.onload = function() {
				...
				if (i == len) {
					var bg = gSelf.baseBgMap;
					bg.style.left = gSelf.baseMap.style.left;
					bg.style.top = gSelf.baseMap.style.top;
					//gSelf.hideMap('bgMap');
					gSelf.displayMap('tile');
					for (var q = bg.childNodes.length; q > 0; q--) {
						var imgtmp = bg.childNodes[q - 1];
						bg.removeChild(imgtmp);
						imgtmp = null;
					}
					//bg.className = "scale-normal";
		...
		setTimeout(function(){getImg();setTimeout(arguments.callee, 15);},15); 
	}
	getImg();
...
gMap.prototype.resetTileInit = function() {
	this.hideMap('tile');
	var d = this.baseMap;
...
gMap.prototype.scaleTileMap = function(mapTool) {
	...
	if (mapTool == "zoomin") {
		//d.className = "scale-animatedIn";
		d.style.webkitTransform="scale(2,2)";
		d.style.transform="scale(2,2)";
		d.style.transition="transform 200ms ease-in-out";
		d.style.transition="-webkit-transform 200ms ease-in-out";
		//d.setAttribute("class", "scale-animatedIn");
	}

	if (mapTool == "zoomout") {
		//d.className = "scale-animatedOut";
		d.style.webkitTransform="scale(0.5,0.5)";
		d.style.transform="scale(0.5,0.5)";
		d.style.transition="transform 200ms ease-in-out";
	    d.style.transition="-webkit-transform 200ms ease-in-out";
		//d.setAttribute("class", "scale-animatedOut");
	...
	if (gSelf.curType == "map") {
		...
		if(Math.abs(gSelf.disMinus)>0){//多点触摸
		    ...
		    if (gSelf.maptool == "zoomto") {
		    	...
		    	var dlty =0;
		    	if(gSelf.moving){
		    		dltx = gSelf.endScrX - gSelf.startScrX;
		    		dlty = gSelf.endScrY - gSelf.startScrY;
		    	}
		    	...
		    	if (Math.abs(dltx) > 0 || Math.abs(dlty) > 0) {
		    		...
		    		var u=setInterval(function(){*/
		    			...
		    			if(q>20){
		    				...
		    				gSelf.xOffset = gSelf.xOffset + dltx;
		    				gSelf.yOffset = gSelf.yOffset + dlty;
		    				var nscx = gSelf.scx - dltx;
		    				var nscy = gSelf.scy - dlty;
...
function worldToScreen(wx, wy) {
	...
	var scx = (gSelf.width)/ 2;
	var scy = (gSelf.height) / 2;
	var scale = mapParams.zoom / (gSelf.width);
...
function calLenAre_MouseDblClick() {
	...
	if (gSelf.maptool == "rulerLength") {
		...
		var area1 = area/1000000;
		var f_x = Math.round(area1*1000)/1000;//保留三位小数
		alert("周长：" + Math.round(perimeter) + "米" +"\n"+"面积：" + Math.round(area) + "平方米"+"，合"+f_x+"平方公里");
	}
	return returnLenArea;
}
...
gMap.prototype.showScale = function(ctx, height) {
  ...
  var result=this.getScaWidth(scale);
  console.log(result);
  ...
  var result={};
    switch(s){
      ...
      case 4:
        result.m = 100;
        result.kmScale ="400米";
		result.halfkmscale ="200";
        break;
      case 8:
        result.m = 100;
        result.kmScale ="800米";
		result.halfkmscale ="400";
        break;
      case 16:
        result.m = 125;
        result.kmScale ="2公里";
